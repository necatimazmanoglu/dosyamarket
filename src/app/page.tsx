...
import { UploadCloud, Share2, Coins } from "lucide-react"; 
import { auth } from "@clerk/nextjs/server"; // Auth kontrolü için gerekli

export default async function LandingPage() {
  // Sunucu tarafında kullanıcı giriş yapmış mı kontrol ediyoruz
  const { userId } = await auth();

  // MANTIK: Kullanıcı giriş yapmışsa -> Ürün Ekleme Paneli
  // Giriş yapmamışsa -> Kayıt Ol Sayfası (/sign-up)
  const targetUrl = userId ? "/dashboard/products/new" : "/sign-up";
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <section className="relative overflow-hidden pt-20 pb-32 lg:pt-32 text-center">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
           ...
           <div className="flex flex-col sm:flex-row gap-4 justify-center">
             ...
             {/* DİNAMİK BUTON: Tasarım aynı, hedef adres akıllı */}
             <Link 
               href={targetUrl} 
      ...
      {/* --- NASIL ÇALIŞIR? (Orijinal Mor Tasarım) --- */}
      ...
      <section className="py-32 text-center relative z-10 overflow-hidden">
         ...
         <div className="max-w-4xl mx-auto px-4">
            ...
            {/* DİNAMİK BUTON */}
            <Link href={targetUrl} className="bg-purple-600 text-white px-12 py-5 rounded-xl font-black text-xl hover:bg-purple-700 transition shadow-xl hover:shadow-purple-200 hover:-translate-y-1">
               Mağaza Aç (Ücretsiz)
            </Link>
         </div>
      </section>
